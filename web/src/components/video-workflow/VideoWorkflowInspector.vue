...
const previewURL = computed(() => String(
  props.node?.config?.preview_url
  || props.node?.output?.preview_url
  || props.node?.output?.url
  || '',
))
  ...
  <aside class="workflow-inspector" aria-label="节点检查器">
    <template v-if="node">
      ...
      <div class="inspector-scroll">
        ...
        <section v-if="imageNode" class="image-section">
          <div class="image-preview">
            <img v-if="previewURL" :src="previewURL" :alt="node.title || '生成图片预览'" />
            <span v-else><Picture /><small>尚未生成图片</small></span>
            <em>288 × 180</em>
          </div>
